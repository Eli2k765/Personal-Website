...
async function scanDirectory(dirPath, relativePath = '') {
    ...
    if (hasBody) {
        // Index the full body for search (Essential for finding Books!)
        // Since we are inside scanDirectory, verify content is available or re-read
        try {
            const contentVal = await fs.readFile(indexMdPath, 'utf8');
            const frontVal = fm(contentVal);
            SEARCH_INDEX.push({
                title: title,
                tags: metadata.tags || [],
                url: node.url,
                content: frontVal.body
            });
        } catch (e) {
            console.error('Error indexing body for:', title, e);
        }
    }
...
function renderTemplate(name, data) {
    ...
    html = repl(html, 'description', data.title + ' - EggByte');
